...
/**
 * @deprecated Este hook usa el endpoint obsoleto /invitees/:id/attendance que ya no existe.
 *
 * Para marcar asistencia, usa directamente:
 * ```typescript
 * await api.patch(`/invitees/${id}/attendance/day`, {
 *   dayNumber: number,
 *   attended: boolean,
 *   notes?: string
 * });
 * ```
 *
 * O implementa la lógica directamente en tu componente como en invitados/page.tsx
 */
export function useMarkAttendance() {
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);

  const markAttendance = async (id: string, data: any): Promise<any | null> => {
    console.warn(
      '[DEPRECATED] useMarkAttendance usa endpoint obsoleto. ' +
      'Usa api.patch(`/invitees/${id}/attendance/day`, { dayNumber, attended, notes? }) en su lugar.'
    );
    ...
    try {
      ...
      // Endpoint deprecado - retornará 404
      const response = await api.patch(`/invitees/${id}/attendance`, data);
      toast.success("Asistencia actualizada correctamente");
      return response.data;
    } catch (err: any) {
      setError(err);
      toast.error(err.response?.data?.message || "Error al marcar asistencia - Endpoint obsoleto");
      return null;
    } finally {
      setIsLoading(false);
    }
